# Rollup Config [![npm](https://img.shields.io/npm/v/@theholocron/rollup-config?color=red)](https://www.npmjs.com/package/@theholocron/rollup-config)

A [Rollup configuration](https://rollupjs.org/guide/en/#configuration-files) for bundling next generation Javascript in the Galaxy.

## Installation

```bash
npm install --save-dev @theholocron/rollup-config
```
...
{
  "scripts": {
    "dist": "rollup -c node:@theholocron/rollup-config --filename=<name>",
  }
}
```
